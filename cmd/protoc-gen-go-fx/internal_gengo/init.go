...
func newFileInfo(file *protogen.File) *fileInfo {
	...
	initMessageInfos := func(messages []*protogen.Message) int {
		i := 0
	...
	walkMessages(f.Messages, func(m *protogen.Message) {
		...
		extN := initMessageInfos(m.Messages)
		if extN > 0 {
			// add ext message to file
			file.Messages = append(file.Messages, m.Messages...)
		}
...
func isTrackedMessage(m *messageInfo) (tracked bool) {
	...
	// annotation proto from protoc-gen-go-fx.
