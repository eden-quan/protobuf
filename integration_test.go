...
func TestIntegration(t *testing.T) {
	...
	for i := range golangVersions {
		...
		if goVersion == golangLatest {
			...
			runGo("ProtocGenGo", command{Dir: "cmd/protoc-gen-go-fx/testdata"}, "go", "test")
		...
		t.Run("BuildRelease", func(t *testing.T) {
			...
			for _, goos := range []string{"linux", "darwin", "windows"} {
				for _, goarch := range []string{"386", "amd64", "arm64"} {
					...
					binPath := filepath.Join("bin", fmt.Sprintf("protoc-gen-go-fx.%v.%v.%v", v, goos, goarch))

					// Build the binary.
					cmd := command{Env: append(os.Environ(), "GOOS="+goos, "GOARCH="+goarch)}
					cmd.mustRun(t, "go", "build", "-trimpath", "-ldflags", "-s -w -buildid=", "-o", binPath, "./cmd/protoc-gen-go-fx")
					...
					suffix := ""
					comment := fmt.Sprintf("protoc-gen-go-fx VERSION=%v GOOS=%v GOARCH=%v", v, goos, goarch)
					switch goos {
					case "windows":
						suffix = ".zip"
						zw := zip.NewWriter(out)
						zw.SetComment(comment)
						fw, _ := zw.Create("protoc-gen-go-fx.exe")
						fw.Write(in)
						zw.Close()
					default:
						suffix = ".tar.gz"
